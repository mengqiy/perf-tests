...
func (ste *simpleExecutor) ExecuteTestSteps(ctx Context, steps []*api.Step) *errors.ErrorList {
	...
	for i, step := range steps {
		fmt.Fprintf(os.Stdout, "!!!!!!!!!!! step name: %v, time: %v !!!!!!!!!\n", step.Name, time.Now())
		fmt.Fprintf(os.Stderr, "!!!!!!!!!!! step name: %v, time: %v !!!!!!!!!\n", step.Name, time.Now())
		// if i == 6 {
		// 	go func() {
		// 		fmt.Fprintf(os.Stdout, "!!!!!!!!!!! step name: %v, time: %v !!!!!!!!!\n", step.Name, time.Now())
		// 		fmt.Fprintf(os.Stderr, "!!!!!!!!!!! step name: %v, time: %v !!!!!!!!!\n", step.Name, time.Now())
		...
		// 		// fmt.Println("!!!!!!!!!!! writing to /tmp/cl2-signal file !!!!!!!!!")
		// 		// err := os.WriteFile("/tmp/cl2-signal", []byte("trigger!"), 0666)
		// 		// if err != nil {
		// 		// 	fmt.Printf("!!!!!!!!!!! ERROR when writing to /tmp/cl2-signal file: %v !!!!!!!!!\n", err)
		// 		// }
		// 	}()
		// }
